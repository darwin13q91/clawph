#!/usr/bin/env python3
"""
Enhanced Gateway with Per-Client Custom Responses
"""
...
class ClientManager:
    ...
    def update_responses(self, cid, responses):
        """Update custom responses for a client"""
        client = self.get(cid)
        if not client:
            return False
...
def generate_reply(client, message_text):
    """Generate reply using client's custom responses"""
    msg = message_text.lower()
    responses = client.get('custom_responses', {})
    
    # Check for keywords
    if any(word in msg for word in ['hi', 'hello', 'hey']):
        return responses.get('greeting', f"Hello! Welcome to {client['business_name']}")
    
    if any(word in msg for word in ['hour', 'open', 'time', 'when']):
        return responses.get('hours', "Please contact us for business hours.")
    
    if any(word in msg for word in ['contact', 'call', 'email', 'phone']):
        return responses.get('contact', "You can reach us through this chat.")
    
    if any(word in msg for word in ['help', 'support', 'assist']):
        return responses.get('help', "I can help with general inquiries. What do you need?")
    
    if any(word in msg for word in ['price', 'cost', 'how much']):
        return responses.get('pricing', "Please contact us for pricing information.")
    
    if any(word in msg for word in ['menu', 'food', 'dish', 'order']):
        return responses.get('menu', "Please check our website or contact us for our menu.")
    
    # Fallback
    return responses.get('fallback', "Thanks for your message! I'll get back to you soon.")
        ...
        'clients': [
            {
                ...
                'status': v['status'],
                'messages': v.get('message_count', 0)
            }
...
def create_client():
    ...
    return jsonify({
        ...
        'config_url': f'https://webhook.amajungle.com/admin/clients/{cid}/config',
        'status': 'created'
    })

@app.route('/admin/clients/<cid>/config', methods=['GET'])
def get_client_config(cid):
    """Get client configuration including custom responses"""
    client = client_manager.get(cid)
    if not client:
        return jsonify({'error': 'not found'}), 404
